...
export const OYI_SURFACE_REGISTRY: Record<IntelligenceSurface, OyiSurfaceDefinition> = {
  ...
  // Precise surfaces ConversationOrchestrator.run() actually receives
  // for the office-internal chat box and the corporate website widget
  // (see officeExport.ts) — mirror `office`'s definition since both are
  // real conversational sub-cases of it, just tagged more precisely for
  // the Oyi Cross-Surface Observability Closure.
  office_internal: { id: "office_internal", label: "Ochiga Office (internal)", memory_scopes: ["office", "lead", "company"], allowed_domains: [...sharedDomains, "leads", "sales", "deployments", "partners"], allowed_actions: ["read", "prepare_confirmed_action"], event_categories: ["office", "lead", "marketing", "sales", "workflow", "support", "conversation"], response_tone: "executive operations assistant", fallback_behavior: "conversation" },
  public_corporate: { id: "public_corporate", label: "Ochiga Website", memory_scopes: ["lead"], allowed_domains: ["lead_qualification", "general_help"], allowed_actions: ["read"], event_categories: ["lead", "marketing", "conversation"], response_tone: "helpful commercial guide", fallback_behavior: "conversation" },
