-- Oyi Facility OS pilot estate onboarding foundation.
-- Safe to run more than once. Stores pilot import/deployment metadata without
-- exposing camera credentials or treating placeholders as live telemetry.
...
create table if not exists facility_cameras (
  id uuid default gen_random_uuid() primary key,
  estate_id uuid not null references estates(id) on delete cascade,
  zone_id uuid references estate_zones(id) on delete set null,
  camera_id text,
  ...
  onvif_port int,
  dvr_nvr_ref text,
  stream_protocol text default 'rtsp',
  rtsp_url text,
  edge_hls_url text,
  username text,
  password text,
  ...
  health_status text not null default 'pending_stream_details',
  metadata jsonb not null default '{}'::jsonb,
  created_by uuid references users(id) on delete set null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint uq_facility_cameras_estate_camera unique (estate_id, camera_id),
  constraint uq_facility_cameras_estate_ip unique (estate_id, ip)
);
