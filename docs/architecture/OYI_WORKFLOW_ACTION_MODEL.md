...
## Programme 4 Phase F — Action Authority Audit

Certified all device-control guarantees this phase's spec calls out, via existing regression suites (all passing, unmodified in this pass): reload restoration, cancellation, confirmation, multigang explicit channel, stale-context protection, command lifecycle monotonicity, confirmation-evidence staleness rejection, provider-event privacy (`smoke:oyi-workflow-action-phase-c-reload`, `smoke:multi-gang-channel-safety`, `smoke:command-lifecycle-truth`, `smoke:oyi-workflow-action-phase-c-multigang`, `smoke:oyi-workflow-action-phase-c-runtime`).

**Found a second, live action-orchestration authority — `src/ai/commandRouter.ts` — reachable via `/watch/command`, `/watch/confirm`, `/watch/cancel` (`src/services/watchAdapterService.ts`) as its primary and only path.** Unlike the `/ai/chat` legacy-fallback scenario (Phase A/B — only reachable when `ConversationOrchestrator` fails to match a capability), Watch never touches `ConversationOrchestrator`/`WorkflowService`/`ActionService` at all; `routeAiCommand` is Watch's sole action pipeline.

**Important correction to the risk assessment**: this is not a fully duplicated device-execution system. Both `oyi-core/domains/devices/deviceActionAdapter.ts` (the canonical path) and `ai/commandRouter.ts` call the exact same `executeDeviceCommandForActor(...)` in `controllers/deviceCommandController.ts` to actually dispatch a device command — the physical execution layer and its execution-truth ledger (`ai_execution_ledger`, already documented above as "the existing device execution truth" even for the canonical path) are genuinely shared and singular. What's duplicated is the layer above that: the conversational orchestration/confirmation state machine. Watch users get `ai/commandRouter`'s own `pending_confirmation`/`confirmed`/`denied` state tracking instead of oyi-core's `WorkflowService`/`ActionService` state machine (draft → awaiting_approval → confirmed → execution_started → execution_result). Two different orchestration layers, one shared executor — not two paths that could produce inconsistent physical outcomes, but two paths with different confirmation UX/audit-trail shapes for the same underlying action.

**Decision (explicitly deferred, not silently skipped)**: migrating Watch onto `ConversationOrchestrator`/`WorkflowService` is real engineering — Watch's request/response contract doesn't match the conversational shape the way `/ai/chat`/`/office/*`/`/communications/*` did for Phase B's routing swaps — and it touches a live physical-device-execution surface with no physical Watch hardware available to verify against in this programme. Per explicit direction, this is documented as the top-priority remaining action-authority gap for a dedicated future pass with real device testing (see Phase N), not attempted blind in Programme 4.

`src/ai/toolRegistry.ts` (`AI_TOOL_REGISTRY`, flagged in Phase D as belonging here) is `ai/commandRouter.ts`'s capability declaration — same scope, same deferral.
